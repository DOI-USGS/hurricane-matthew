...
visualize.matthew_map <- function(viz){
  ...
  xml_add_child(svg, 'rect', x='575', y='5', width="135", height='40', fill='white', opacity='0.4')
  xml_add_child(svg, 'text', x='640', y='30', 'USGS stream', dy='-0.5em', 'text-anchor'='middle')
  xml_add_child(svg, 'text', x='640', y='30', 'gage discharge', dy='0.5em', "text-anchor"='middle')
  
  ys <- seq(40,as.numeric(vb[4])-20, length.out = cnt)
  cnt = 0;
  for (i in 1:length(gages)){ # FRAGILE - assumes all gages are on the map!!
    ...
    if (!is.null(svg.points) && !is.na(svg.points[1])){
      ...
      g.dot <- xml_add_child(svg, 'g', transform=sprintf('translate(%s,%s)', "570", ys[cnt])) # xml_attr(cr[i], 'cx'), xml_attr(cr[i], 'cy')
      xml_add_child(g.dot, 'polyline', points = svg.points[1], class='sparkline', id=paste0('sparkline-',gages$site_no[i])) #class='hidden', 
    }
  ...
  xml_add_child(g.legend, 'circle', cx = as.character(as.numeric(box.w)/2), r='2', class='nwis-dot', cy = as.character(as.numeric(ys[i])+53), class='nwis-legend-dot')
  ...
  xml_add_child(svg, 'text', ' ', id='timestamp-text', class='time-text', "490", y="320", 'text-anchor'="middle")
